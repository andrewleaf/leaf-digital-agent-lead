...
- [ ] Root `package.json` exists with name `localdraft`, `private: true`, `type: "module"`, `packageManager: "pnpm@9.15.9"`, and `engines.node` satisfying every tool in the manifest.
- [ ] Scripts cover `dev`, `build`, `start`, `lint`, `typecheck`, `format`, `test`, `test:watch`, and Drizzle Kit (`db:generate`, `db:migrate`, `db:push`, `db:studio`).
- [ ] Every dependency in the manifest traces to a named technology or requirement on the board (see Dependency rationale below).
- [ ] Runtime libraries are in `dependencies`; build, lint, test, and type-only tooling is in `devDependencies`; keys are sorted alphabetically in both blocks.
- [ ] Every package name and version range resolves against the npm registry (`npm view <pkg> version`) — no invented versions.
- [ ] Stack technologies rejected in the architecture epic (Remix/React Router v7, SvelteKit, Material UI, Chakra UI, Emotion, Prisma, PostgreSQL, MySQL) are absent.
- [ ] No packages installed: no `node_modules`, no lockfile, and no application scaffolding beyond `package.json`.
