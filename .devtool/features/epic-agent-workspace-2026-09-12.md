...
modified: "2026-09-16T22:42:48.777Z"
completedAt: null
labels: ["epic"]
order: "ac"
---
...
StatusDefinition & System MeaningTransition Trigger`New`Business discovered from local search; enrichment has not yet run.Initial campaign ingestion`Enriched`Public website parsed; facts, CTAs, and contact emails captured.Scraper & fact engine complete`Drafted`Personalized email draft (subject + body) generated from facts.LLM draft generation complete`Needs edit`Agent flagged draft for weak observations, awkward phrasing, or poor offer alignment.Agent manual review flag`No email`No valid `mailto:` or contact page email could be extracted.Harvester yields empty address list`Skipped`Account disqualified as out-of-ICP, competitor, or duplicate.Agent manual triage / soft filter`Ready`Draft reviewed and approved by human agent; pending dispatch.Agent approves draft`Sent`Agent copied or dispatched draft via mail client; sent timestamp saved.Agent clicks "Mark Sent"`Replied`Recipient business replied to outreach (positive or negative).Agent logs inbound reply`Follow-up due`Next-touch follow-up date has been reached or is overdue.System date matches next-touch date
